...
public class PlaylistAPI extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (session.getAttribute("currentMemberID") != null || true){
...
//                    Query<Playlist> query = ofy().load().type(Playlist.class).filter("createdBy", member.getId()).filter("status", 1);
                    Query<Playlist> query = ofy().load().type(Playlist.class).filter("createdBy", 0).filter("status", 1);
